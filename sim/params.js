...
module.exports = {
  ...
  prestige: { pA: 0, pD1: 200000, pB: 11, pD2: 10000, pG: 0.075, pMin: 10000, firstCost: 100000000, costCpsMul: 500,
  ...
  quota: {
    ...
    reachGainFrac: 0, reachCoef: 1.2, // gainFrac=0で無効(2026-07-11実測: gain基準0.9はT3b 169/192で時間基準と同着=残NGは基礎ノルマの中盤未達でありreach設計では直らない。コードは温存) reachPow: 10, reachMinSec: 600, reachMaxSec: 6000, reachEmaAlpha: 0.35 // reachGainFrac(2026-07-11 第2再設計): 未達=獲得予定PT≥次スキル最安×0.9(時間比は0設定時のフォールバック) // reachCoef 2.0→1.2(2026-07-11 T3a廃止後: 未達はT3b専用になったのでρ*0.93→0.98へ後ろ倒し。前回より長い周回の維持率70-79%割れを救う。掃引: S4 33→38・S9 35→41・S3 44→46・S1 43→44)
  },
  ...
  huntDirect:   { coef: 0.15, stagePow: 0.5, countPow: 1.4, ref: 30,  startStage: 5, satMax: 15, otherMul: { click: 0.15, balanced: 0.15, golden: 0.9, default: 0.3 }, peddlerFrac: 0.06 }, // peddlerFrac 0.02→0.06(2026-07-11: hunt序盤=portalNetwork段2前の金50-60%NGを行商の先行分で立てる。(a)31→42/44・②改不変) // otherMul.golden 0.6追加(2026-07-11 ②改golden: NG周回の共通形=討伐2-4%が幾何平均を崩す→金を絞らず討伐を立てる。②改10→16/44・(a)44→42/44) // otherMul方針別マップ(2026-07-10): click/balancedは0.15(各+1周回)・bake/goldenは従来0.3(0.15に下げると設が押されbake②改−15=C1実測) // 投資量=討伐perk合計8種(㉘討伐≥30%へ増幅・投資連動=狩猟専の周回だけ強く効く)。ベース=金クッキー期待収入率(金経済連動)。satMax=高投資周回(perk1000+)の独走を飽和で抑え②改と両立
  tapDirect:    { coef: 0.01, stagePow: 0.5, countPow: 2, ref: 20,  startStage: 5, clickBonus: 5.0, satMax: 150, anchorGolden: 0.5, stallFrac: 0.05, otherMul: { golden: 0.6, balanced: 7.0, default: 1 } }, // balanced 5.5→7.0(2026-07-11: 金は金を呼ぶの金インフレで後半打8-9%が再発=31→35/49) // balanced 4.0→5.5(2026-07-11: reach1.2の周回延長で後半打<10%が再発=33/49→38/49) // clickBonus3→5+satMax0→150(2026-07-11: 単独では両立不可(clickBonus増=②改崩れ・satMax単独=中盤も削る)だが複合で「中盤+22%・後半飽和」となりclick(a)25→29/49・②改46/49=純増+2) // otherMul.balanced 4.0(2026-07-11: balanced後半の打3-6%<10%対策=タップ直送を厚く。②改はbalanced対象外なので安全。(a)29→37/49) // clickBonus=3(2026-07-10採用): click中盤(神指0・指のみ=inv100-280)の打直2-12%を×3し打≥30%へ(click23→30/48)。後半(神指1851+)は既に②改NGなので失うものなし。satMaxは中盤/後半のinv比40-100倍を両立できず不採用。anchorGolden=0.5(採用)=**神指登場前だけ**max(base,0.5×金相場)(balanced中盤run25-32の打4-8%→14-18%で36→44/48。常時適用は後半打85%爆発=E2/E3実測)。otherMul.golden=0.6(採用)=golden後半の打圧迫を絞り41/44 // 投資量=神の指+強い指/10。clickBonus(第12次R続き・検証中)=click方針だけ厚く(bankDirectと同型・増加方向)。1で従来どおり
  ...
  res: {
    ...
    ovenSelf: 30, ovenOwn: 0.067, ovenStage: 0.03, // 0.012→0.03(2026-07-11 ①oven: surge減速で直送比のcpsが痩せ中盤以降のliftが1.02-1.12に沈む→層ランプで再係留。S10 NG9/23→0/24 min1.296)
